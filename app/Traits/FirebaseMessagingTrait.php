...
trait FirebaseMessagingTrait
{
    ...
    private function sendOrderFirebaseNotification(
        ...
        if($deviceTokens == null || $topic != null){
            $message = CloudMessage::withTarget('topic', $topic)
                ->withNotification($notification) // optional
                ->withData($data ?? []); // optional
        }else{
        ...
        if ($deviceTokens != null && is_array($deviceTokens) && count($deviceTokens) > 0) {
            ...
            // Log the results
            Log::info("Push notification sent. Success: {$successCount}, Failures: {$failureCount}");

            // Handle individual failures (optional)
            foreach ($report->failures() as $failure) {
                Log::error("Notification failed for token {$failure->target()}: {$failure->error()->getMessage()}");
            }
        } else {
            $messaging->validate($message);
            $messaging->send($message);
    ...
    public function sendOrderNotificationToDrivers(Order $order, $driverIds = [])
    {
        ...
        $this->sendOrderFirebaseNotification(
            $driverIds,
            __("Order Update"),
            __("Order #") . $order->code . __(" has been requested to you"),
            $orderData
        );
        $this->resetLocale();
    }
