...
class VendorTypeTable extends OrderingBaseDataTableComponent
{
    ...
    public function columns(): array
    {
        ...
        return [
            ...
            $this->colorColumn(),
            Column::make(__('Show on Homepage'))->format(function ($value, $column, $row) {
                return view('components.toggle', ['model' => $row, 'field' => 'show_on_homepage']);
            }),
            Column::make(__('Show store on Homepage'))->format(function ($value, $column, $row) {
                return view('components.toggle', ['model' => $row, 'field' => 'show_store_on_homepage']);
            }),
    ...
    public function toggleStatus($id, $field)
    {
        ...
        if ($vendorType) {
            $vendorType->{$field} = !$vendorType->{$field};
            $vendorType->save();
            $this->emit('refreshTable');
            session()->flash('message', __('Show on homepage status updated successfully.'));
        } else {
            session()->flash('error', __('Show on homepage not found.'));
        }
    }
}
