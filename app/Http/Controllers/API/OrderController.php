...
class OrderController extends Controller
{
    ...
    {
        \Log::info('order request payload => ' . json_encode($request->all()));
        ...
        $validator = Validator::make($request->all(), [
            ...
            // 'delivery_fee_type' => 'required|string|in:below_100,between_101_200,above_300',
        ...
        try {
            ...
            if ($userHasSubscription) {
                ...
                $deliveryFee = 0;
                $deliveryFeeType = 'subscription_free';
            } else {
                // Regular fee logic
                if ($deliveryZone) {
                    if ($deliveryZone->delivery_fee !== null) {
                        // base fee
                        $deliveryFee = $deliveryZone->delivery_fee;
                        $deliveryFeeType = 'base';
                    } else {
                        // tiered fee
                        $rules = json_decode($deliveryZone->delivery_fee_rules, true);
            ...
            // merge computed fee back into request so downstream code can use it
            $request->merge([
