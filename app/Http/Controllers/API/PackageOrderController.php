...
class PackageOrderController extends Controller
{
    ...
    public function fetchVendors(Request $request)
    {
        ...
        try {
            ...
            //     ->active()
            //     ->get();
            $vendors = Vendor::with('package_types_pricing')
            ...
            foreach ($vendors as $vendor) {
                ...
                $vendorArray = $vendor->toArray(); // ✅ Proper conversion

                if ($vendorServiceAllLocations) {
                    $vendorsWithAllLocations[] = $vendorArray;
                } else {
                    $vendorArray['unsupported_locations'] = $unSupportedLocations;
                    $vendorsWithAtLeastOneLocation[] = $vendorArray;
                }
            }

            if ($request->boolean('correct_only', true)) {
                return response()->json([
                    "vendors" => $vendorsWithAllLocations,
                ], 200);
            }

            $allVendors = array_merge($vendorsWithAllLocations, $vendorsWithAtLeastOneLocation);
    ...
    {
        \Log::info('payload summary parcel'. json_encode($request->all()));
        ...
        try {
            ...
            if (!empty($request->stops)) {
                ...
                if ($dropoffLocation && $dropoffLocation->latitude && $dropoffLocation->longitude) {
                    $distanceRecord = DeliveryAddress::distance(
                    ...
                    $deliveryLocationDistance = $distanceRecord->distance ?? 0;
                } else {
                    // If latitude/longitude missing, skip calculation but return 0
                    $deliveryLocationDistance = 0;
                }
            }
            ...
            if ($packageTypePricing && $packageTypePricing->price_per_kg) {
                // Vendor has package type pricing and it's per KG
                $sizeAmount = ($packageTypePricing->size_price ?? 0) * $weight;
            } elseif ($packageTypePricing) {
                // Vendor has package type pricing but not per KG
                $sizeAmount = $packageTypePricing->size_price ?? 0;
            } else {
                // Vendor has no package type pricing at all — use default or 0
                $sizeAmount = 0;
            }


            //calculate the distance price
           // Ensure $deliveryLocationDistance is always a number
            $deliveryLocationDistance = $deliveryLocationDistance ?? 0;

            // Handle distance pricing safely
            if ($packageTypePricing && $packageTypePricing->price_per_km) {
                // Vendor has per km pricing
                $distanceAmount = ($packageTypePricing->distance_price ?? 0) * $deliveryLocationDistance;
            } elseif ($packageTypePricing) {
                // Vendor has flat distance price
                $distanceAmount = $packageTypePricing->distance_price ?? 0;
            } else {
                // No vendor pricing at all — default 0
                $distanceAmount = 0;
            }

            // Always add base price if available
            $distanceAmount += $packageTypePricing->base_price ?? 0;

            // Multiple stop fee (safe even if stops or fee is missing)
            if (!empty($request->stops) && is_array($request->stops) && count($request->stops) > 2) {
                $distanceAmount += (($packageTypePricing->multiple_stop_fee ?? 0) * count($request->stops));
            }
            ...
            if (!empty($request->coupon_code)) {
                ...
                //
            $tax = (float) ($tax ?? 0);  // Ensure $tax is numeric
            ...
            if ($vendor && !empty($vendor->fees)) {
                foreach ($vendor->fees as $fee) {
                    $feeAmount = 0;
                    if ($fee->percentage) {
                        $feeAmount = ($fee->value / 100) * $subTotalAmount;
                    } else {
                        $feeAmount = $fee->value;
                    }
    ...
    public function isStopServiceByVendor($vendorId, $location)
    {
        ...
        $lng = $location['longitude'] ?? ($location['lng'] ?? ($location['long'] ?? null));
        try {
            //check iof city is even in the system
            $found = $this->vendorCityLocationCheck($city, $country, $lat, $lng, $vendorId);
            if ($found) {
