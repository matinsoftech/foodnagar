...
class RegularOrderService
{
    ...
    protected function calculateDeliveryFee($subTotal, $deliveryFeeType)
    {
        switch ($deliveryFeeType) {
            ...
            case 'above_300':
                return 0;
            default:
                // fall back to base delivery fee
                return config('app.base_delivery_fee', 0);
        }
    }
    ...
    public function singleOrder(Request $request)
    {
        ...
        if (allowOldUnEncryptedOrder()) {
            ...
            // $deliveryFeeType = $request->delivery_fee_type ?? null;

            // $order->delivery_fee = $this->calculateDeliveryFee(
            //     $orderData['sub_total'], // or $orderData['total']
            //     $deliveryFeeType
            // );
        ...
        if ($request->has('visiting_guests')) {
            foreach ($request->visiting_guests as $guest) {
                $identityProofPath = null;

                if (isset($guest['identity_proof']) && $guest['identity_proof'] instanceof \Illuminate\Http\UploadedFile) {
                    $identityProofPath = $guest['identity_proof']->store('order_guests', 'public');
                }
