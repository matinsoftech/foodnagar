...
class OTPService
{
    ...
    public function sendOTP($phone, $message, $gateway = null, $user = null, $code = null): ?String
    {
        ...
        $enabledSmsGateway = setting('otpGateway', 'sparrow');
        if ($gateway != null) {
        ...
        Log::info("SMS Sending", [$phone, $message, $gateway, $enabledSmsGateway]);
        ...
        if ($enabledSmsGateway == "twilio") {
        ...
        } else if ($enabledSmsGateway == "sparrow") {
            $authkey = env("SPARROW_AUTHKEY");
            ...
            $apiEndpoint = 'http://api.sparrowsms.com/v2/sms/';
            $payload = [
                "token" => $authkey,
                "to" => $phone,
                "from" => $sender,
            ...
            if (!$response->successful()) {
                logger("Sparrow Error", [$response->json()]);
