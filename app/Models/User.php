...
class User extends Authenticatable implements HasMedia
{
    ...
    public function getLocationNameAttribute()
    {
        ...
        $cacheKey = "reverse_geocode_{$this->latitude}_{$this->longitude}";

        return cache()->remember($cacheKey, now()->addHours(12), function () {
            try {
                $response = file_get_contents("https://nominatim.openstreetmap.org/reverse?format=json&lat={$this->latitude}&lon={$this->longitude}&zoom=14&addressdetails=1");
                dd($response);
                $data = json_decode($response, true);
                return $data['display_name'] ?? 'Unknown location';
            } catch (\Exception $e) {
                \Log::info('location error: ' . $e->getMessage());
    ...
    {
        return $this->hasOne(\App\Models\SubscriptionPurchase::class, 'user_id')
            ->latestOfMany(); // Gets the most recent subscription
    }
