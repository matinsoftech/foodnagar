...
class VPSTaxiOrderMatchingJob implements ShouldQueue
{
    ...
    public function handle()
    {
        try{
            ...
            Log::channel('single')->info("Order loaded ==> " . $this->order->code);

            $order = $this->order;
            $order->refresh();
            // Log::info("Order loaded ==> " . $order->code . "");
            logger("Order loaded ==> " . $order->code . "");

            
            error_log("Order loaded ==> " . $this->order->code);
            file_put_contents(storage_path('logs/debug.log'), "Order loaded ==> " . $this->order->code . PHP_EOL, FILE_APPEND);
            ...
            logger("Order detail" . $order);
            logger("Order can auto assign driver" . $order->taxi_order);
            ...
            if ($order->can_auto_assign_driver) {
                ...
                try {
                    ...
                    // Log::info("Drivers found for order =>" . $order->code . "", [$driverDocuments]);
                    ...
                    logger("Order error line", [$ex->getLine()]);
                }
            }
            ...
            logger("Error in VPSTaxiOrderMatchingJob", [$e->getMessage() ?? $e]);
            // Log::info("Error in VPSTaxiOrderMatchingJob", [$e->getMessage() ?? $e]);
        }
