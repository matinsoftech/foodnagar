...
@endpush
@section('content')
@section('title', __('SubSubCategory'))
<x-baseview title="{{ __('SubSubCategory') }}" >
    <div class="container mx-auto py-6">
        <!-- Create Blog Button -->
        <div class="mb-4 text-right">
            <a href="{{route('subsubcategory.create')}}"  class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">SubCatgeory</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">Sub-SubCategory</th>
                ...
                <tbody>
                    @forelse ($subsubcategories as $subsubcategory)
                        <tr class="border-t border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $subsubcategory->category->name }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $subsubcategory->subcategory->name }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $subsubcategory->name }}</td>
                           
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $subsubcategory->created_at->format('Y-m-d') }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">
                                @if ($subsubcategory->is_active == 1)
                                <button class="px-4 py-2 text-white bg-green-500 rounded-md">Active</button>
                            @else
                                <button class="px-4 py-2 text-white bg-gray-500 rounded-md">Inactive</button>
                            @endif
                            </td>
                            ...
                            <td class="px-6 py-3 text-center">
                                <div class="flex justify-center space-x-2">
                                    <!-- View Button -->
                                    <a href="{{ route('subsubcategory.show', $subsubcategory->id) }}"
                                    ...
                                    <!-- Delete Button -->
                                    <form action="{{ route('subsubcategory.destroy', $subsubcategory->id) }}" method="POST" class="inline" id="deleteForm{{ $subsubcategory->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
