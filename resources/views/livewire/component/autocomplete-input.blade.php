<div>
    <label class="block mt-4 text-sm">
        ...
        <div class="flex items-center">
            <input class='block w-full p-2 mt-1 text-sm border border-gray-300 rounded focus:border-primary-400 focus:outline-none focus:shadow-outline-primary' id="{{ 'autocomplete-'.
                ($selectedId ?? 'new') }}" autocomplete="off" placeholder="{{ $placeholder ?? __('Search ').__(class_basename($model)) }}" type="{{ $type ?? 'text' }}" wire:model.debounce.700ms='name' {{ $disable ?'disabled':'' }} />
            @if ($showClear ?? false)
