...
<div class="pt-4 mt-4 border-t">
    <div class="font-bold text-md mb-2">
        <x-details.item title="{{ __('Checkin Date') }}"
        text="{{ $selectedModel != null ? $selectedModel->check_in_date : '' }}" />
    </div>
    <div class="font-bold text-md mb-2">
        <x-details.item title="{{ __('Checkout Date') }}"
        text="{{ $selectedModel != null ? $selectedModel->check_out_date : '' }}" />
    </div>
    <div class="font-bold text-md mb-2">
        <x-details.item title="{{ __('Number Of Guests') }}"
        text="{{ $selectedModel != null ? $selectedModel->number_of_guests : '' }}" />
    </div>
</div>

{{-- Visiting Guests --}}
@if($selectedModel != null && $selectedModel->guests->count() > 0)
    <div class="mt-5 border-t">
        <p class="font-bold text-md mb-2">{{ __('Guests') }}</p>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            @foreach($selectedModel->guests as $guest)
                <x-details.item title="{{ $guest->name }}">
                    <p class="text-sm font-medium"><span class="font-bold">{{ __('Age') }}:</span> {{ $guest->age ?? '--' }}</p>
                    <p class="text-sm font-medium"><span class="font-bold">{{ __('Gender') }}:</span> {{ ucfirst($guest->gender ?? '--') }}</p>

                    @if($guest->identity_proof)
                        <p class="text-sm font-medium"><span class="font-bold">{{ __('Identity Proof') }}:</span></p>
                        <a href="{{ asset('storage/'.$guest->identity_proof) }}" target="_blank">
                            <img src="{{ asset('storage/'.$guest->identity_proof) }}" alt="{{ $guest->name }}" class="w-36 h-36 rounded-sm mt-1"/>
                        </a>
                    @else
                        <p class="text-sm font-medium">{{ __('Identity Proof Not Provided') }}</p>
                    @endif
                </x-details.item>
            @endforeach
        </div>
