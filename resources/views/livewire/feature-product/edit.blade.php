...
@section('content')
@section('title', __('Edit Feature Section'))

<x-baseview title="{{ __('Edit Feature Section') }}" >
    <div class="overflow-x-auto">
        <div class="container mx-auto py-6">
            <div class="bg-white shadow-md rounded-lg p-6">
                <form action="{{ route('feature_product.update', $feature_product->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" required
                            ...
                            value="{{ old('title', $feature_product->title) }}">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" required
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">{{ old('description', $feature_product->description) }}</textarea>
                    </div>
                        ...
                        <select id="product_id" name="product_id[]" multiple
                            ...
                            @foreach ($product as $products)
                                <option value="{{ $products->id }}" 
                                    {{ in_array($products->id, json_decode($feature_product->product_id) ?? []) ? 'selected' : '' }}>
                                    {{ $products->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="priority" id="priority" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                            value="{{ old('priority', $feature_product->priority) }}">
                    </div>
