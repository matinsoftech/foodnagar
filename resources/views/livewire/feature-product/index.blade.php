...
@endpush
@section('content')
@section('title', __('Feature Section'))
<x-baseview title="{{ __('Feature Section') }}" >
    <div class="container mx-auto py-6">
        <!-- Create Blog Button -->
        <div class="mb-4 text-right">
            <a href="{{route('feature_product.create')}}"  class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">Priority</th>
                ...
                <tbody>
                    @foreach ($feature_product as $productFeatured)
                        <tr class="border-t border-gray-200 hover:bg-gray-50">
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $loop->iteration}}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $productFeatured->title }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">
                                {{ Str::limit($productFeatured->description, 100, '...') }}
                            </td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $productFeatured->priority }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $productFeatured->created_at->format('Y-m-d') }}</td>

                            <td class="px-6 py-3 text-center">
                                <div class="flex justify-center space-x-2">
                                    <!-- View Button -->
                                    <a href="{{ route('feature_product.show', $productFeatured->id) }}"
                                        class="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-xs">
                                        <i class="fas fa-eye mr-1 text-sm"></i> <!-- Adjust icon size -->
                                    </a>

                                    <!-- Edit Button -->
                                    <a href="{{ route('feature_product.edit', $productFeatured->id) }}"
                                    ...
                                    <!-- Delete Button -->
                                    <form action="{{ route('feature_product.destroy', $productFeatured->id) }}" method="POST" class="inline" id="deleteForm{{ $productFeatured->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            class="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600 text-xs delete-btn"
                                            data-id="{{ $productFeatured->id }}">
                    ...
                    @endforeach
                    @if ($feature_product->isEmpty())
                        <tr>
                            <td colspan="6" class="text-center py-4">No feature section found.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
