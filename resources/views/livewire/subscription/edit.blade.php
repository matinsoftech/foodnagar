    ...
    <div class="w-full bg-gray-50 min-h-screen py-10">
        <div class="max-w-4xl mx-auto bg-white shadow-md rounded-xl p-8">
            ...
            <div class="flex items-center justify-between border-b border-gray-200 pb-5 mb-6">
                <h2 class="text-2xl font-bold text-gray-800">Edit Subscription Package</h2>
            ...
            <!-- Form -->
            <form action="{{ route('subscription.update', ['subscription' => $package->id]) }}" method="POST"
                ...
                <div>
                    ...
                    <select id="vendor_type_ids" name="vendor_type_id[]" multiple
                        class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 h-32">
                        @foreach ($vendorTypes as $type)
                            <option value="{{ $type->id }}"
                                {{ in_array($type->id, old('vendor_type_id', $package->vendor_type_id ?? [])) ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                    {{-- <p class="text-gray-500 text-sm mt-1">Hold Ctrl (Windows) / Cmd (Mac) to select multiple.</p> --}}
                </div>
                ...
                <div>
                    ...
                    @if ($package->photo)
                        <img src="{{ asset('storage/' . $package->photo) }}" alt="Current photo"
                            class="mt-3 h-28 w-auto rounded-lg border border-gray-200"
                            style=" width: 10rem;>
@endif
            </div>
