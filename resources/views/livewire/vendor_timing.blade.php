<div>
    <x-modal-lg confirmText="{{ __('Save') }}" action="saveDays" :clickAway="false">
        ...
        <div class="space-y-1">
            ...
            <x-buttons.primary title="{{ __('Always Open') }}" type="button" wireClick="alwaysOpen" />
        </div>
            
        <div class="flex items-center py-3 mt-5 gap-2 border-t border-b">
            <div class="w-4/12">{{ __('Day') }}</div>
            <div class="w-4/12">{{ __('Slot') }}</div>
            <div class="w-4/12">
                {{ __('Openning Time') }}
            </div>
        ...
        @if (!empty($workingDays))
            @foreach ($workingDays as $key => $workingDay)
                <div class="flex items-start my-1 pb-3 gap-2 border-b">
                    ...
                    <div class="w-4/12">
                        <x-input title="" type="number" name="workingDays.{{ $key }}.slot"
                            noMargin="true" placeholder="Slot" />
                    </div>
                    <div class="w-4/12">
