...
<x-baseview title="{{ __('Edit Brand') }}" >
                ...
                <form action="{{ route('brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="vendor_type_id" required
                            ...
                            @foreach ($vendorTypes as $vendorType)
                                <option value="{{ $vendorType->id }}" {{ old('vendortype', $brand->vendor_type_id) == $vendorType->id ? 'selected' : '' }}>
                                    {{ $vendorType->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        <label class="inline-flex items-center text-sm font-medium text-gray-700">
                            <input type="checkbox" name="is_published" {{ $brand->is_active ? 'checked' : '' }}
                                class="form-checkbox text-blue-500 focus:ring-blue-500">
                            <span class="ml-2">Publish this brand</span>
                        </label>
                    </div>

                    <!-- Action Buttons -->
                    <div class="flex justify-end mt-4">
                        <a href="{{ route('brands.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">
                            Cancel
                        </a>
