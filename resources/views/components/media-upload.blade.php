...
<label class="block mt-4 text-sm">
    ...
    <div
        ...
        <div x-show="!isUploading">
            ...
            {{-- File picker --}}
            <input type="file" class="hidden" accept="{{ $rules ?? '' }}" {{ $multiple ? 'multiple' : '' }}
                @if ($defer)
            ...
            @if (!empty($photo))
                <div class="flex items-center space-x-4">
                    ...
                    <div class="font-light text-gray-500">
                        ...
                        @endif
                        <button wire:click="$set('{{ $name }}', null)" class="px-2 mt-2 text-xs text-red-400 border border-red-400 rounded">
                            {{ __('Remove') }}
                        </button>
                    </div>
                </div>

            {{-- When showing existing preview --}}
            ...
            {{-- Empty state --}}
            @else
                <p class="flex items-center text-sm font-light text-gray-400 cursor-pointer">
